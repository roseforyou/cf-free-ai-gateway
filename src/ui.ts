...
<body>
  <main class="wrap">
    <section class="hero">
      ...
      <div class="actions"><a class="btn" href="/v1/models">查看模型列表</a><a class="btn secondary" href="/health">健康检查</a><button class="secondary" onclick="copyText('${baseUrl}')">复制 Base URL</button></div>
    </section>
    ...
    <section class="grid">
      <div class="card"><h3>Chat</h3><p><code>/v1/chat/completions</code><br/>兼容 OpenAI Chat Completions。</p></div>
      <div class="card"><h3>TTS</h3><p><code>/v1/audio/speech</code><br/>把 Cloudflare Aura 包成 OpenAI TTS 风格。</p></div>
      <div class="card"><h3>STT</h3><p><code>/v1/audio/transcriptions</code><br/>multipart 上传音频，返回文字。</p></div>
      <div class="card"><h3>Embedding</h3><p><code>/v1/embeddings</code><br/>适合检索、RAG 和语义搜索。</p></div>
      <div class="card"><h3>Image</h3><p><code>/v1/images/generations</code><br/>图像生成返回 base64。</p></div>
      <div class="card"><h3>Raw Run</h3><p><code>/cf/run/:model</code><br/>直接透传任意 Cloudflare 模型。</p></div>
    </section>
    <section class="section tester"><div class="card"><h3>在线测试 Chat</h3><p style="color:var(--muted)">填入 API_KEY，测试当前 Worker 是否可用。</p><input id="key" placeholder="API_KEY" /><textarea id="prompt">你好，用中文介绍一下你自己。</textarea><button onclick="testChat()">发送测试</button></div><div class="card"><h3>返回结果</h3><pre class="result" id="result">等待测试...</pre></div></section>
